...
._total_header { text-align: center }
...
._add { position: absolute; left: 198px; top: 4px; padding: 8px; border-radius: 4px; }
...
._timerecord::after {
    content: attr(data-time-entry-id);
    color: #666;
    font-size: 8px;
    display: block;
    position: absolute;
...
`);
    ...
    function safeGtag(event, action, attributes) {
        if(data.isDemo === true) {
            return;
        }
        gtag(event, action, attributes);
    }
    ...
    function addButtonClicked(event) {
        ...
        const selectedItems = document.body.querySelectorAll(cssSelectors.timesheetSelected);
        ...
        safeGtag('event', count == 1 ? 'mytime-paste-single' : 'mytime-paste-multiple', {
            'event_category' : 'mytime',
            'event_label' : count == 1 ? 'Saved a single timesheet' : 'Saved multiple timesheets',
            'value': count
        });
        return false;
    }
    ...
    function deleteButtonClicked(event) {
        ...
        safeGtag('event', 'mytime-timesheet-delete-button', {
            'event_category' : 'mytime',
            'event_label' : 'Timesheet delete button clicked',
        });
    }
    ...
    function deleteKeyPressed(withoutConfirmation) {
        ...
        safeGtag('event', 'mytime-timesheet-delete-keypress', {
            'event_category' : 'mytime',
            'event_label' : withoutConfirmation ? 'Timesheet delete key pressed (Shift key - no confirmation)' : 'Timesheet delete key pressed (with confirmation)',
        });
    }
    ...
    function editHourButtonClicked(event) {
        ...
        safeGtag('event', 'mytime-timesheet-edit-duration-button', {
            'event_category' : 'mytime',
            'event_label' : 'Timesheet edit duration button clicked',
        });
    ...
    function setHourButtonClicked(event) {
        ...
        safeGtag('event', 'mytime-timesheet-save-duration-button', {
            'event_category' : 'mytime',
            'event_label' : 'Timesheet save duration button clicked',
    ...
    function editHoursKeyPressed(event) {
        ...
        safeGtag('event', 'mytime-timesheet-save-duration-keypress', {
            'event_category' : 'mytime',
            'event_label' : 'Timesheet save duration key pressed',
            'value' : duration
        });
    }
    ...
    function registerTimesheetEvents() {
     ...
     function updateTimeEntryElement(incompleteRecord, item) {
     ...
     function updateIncompleteElements() {
         ...
         loadTimeDateByDate(dateId, dateId, function(json) {
             for(let item of json.items) {
                ...
                if(item.dateUtc === dateId && item.taskId === firstRecord.dataset.taskId) {
                    updateTimeEntryElement(firstRecord, item);
                }
             };
             updateTotals();
         });
         ...
         if(incompleteRecords.length > 0) {
             setTimeout(updateIncompleteElements, 300);
         }
     }
   ...
   function loadPublicHolidays() {
    ...
    function showUkPublicHolidays(json) {
        if(json && json["england-and-wales"] && json["england-and-wales"].events) {
            ...
            gtag('event', 'mytime-holidays-loaded', {
                'event_category' : 'mytime',
                'event_label' : 'Timesheet UK Holidays loaded',
                'value' : holidaysDisplayed,
    ...
    async function showIntro() {
     ...
     function createTimeDataLayoutHtml() {
         ...
         '    <div class="xui-column-1-of-12">' +
         '      <span class="xui-heading-small">Date</span>' +
         '    </div>' +
         '    <div class="xui-column-2-of-12 _total_header">' +
         '      <span class="xui-heading-small">Total Hours</span>' +
         '    </div>' +
    ...
    function loadAppData() {
         // or fetch https://go.xero.com/api/projects/app-data
